-- Align search composition helpers with canonical client_name and ensure
-- re-embed/reindex flows use the same public search contract.

CREATE OR REPLACE FUNCTION compose_project_embedding_text(p_id UUID)
RETURNS TEXT AS $$
SELECT trim(concat_ws(' ',
    COALESCE(p.name, ''),
    COALESCE(NULLIF(p.client_name, ''), p.brand, ''),
...
SELECT
    ...
    setweight(to_tsvector('simple', unaccent(COALESCE(NULLIF(p.client_name, ''), p.brand, ''))), 'A') ||
...
WHERE p.id = p_id
GROUP BY p.id, p.name, p.client_name, p.brand, p.description, pp.solution_summary,
